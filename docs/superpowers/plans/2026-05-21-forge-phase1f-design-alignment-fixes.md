...
## Task 6: Phase 1e — Multi-Platform Manifest Management

**Problem**: Manifest generation is baked into `forge init` only. No way to add/remove platforms after init, or reinstall manifests.

**Files to create/modify:**
- `forge/src/commands/manifest.ts` (new) — manifest management
- `forge/tests/unit/commands/manifest.test.ts` (new) — tests
- `forge/src/index.ts` — register `manifest` command

**Steps:**
1. Create `manifest.ts` with `runManifest(projectRoot, subcommand, options)`:
   - `generate` — regenerate all manifests from config.json platforms
   - `add <platform>` — add a platform manifest
   - `remove <platform>` — remove a platform manifest
   - `list` — show installed platform manifests
2. Reuse `generateManifest` logic from `init.ts` (extract to shared utility)
3. Write tests, run, commit

---

## Self-Review Checklist

1. All 6 fixes match design doc exactly
2. No new features beyond what design doc specifies
3. All existing tests still pass
4. New tests cover all new code paths
5. No breaking changes to existing CLI commands
