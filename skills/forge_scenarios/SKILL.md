...
This skill produces the scenario artifact from explicit inputs. It does not own
Forge Runtime state and must not infer feature state from `.forge/progress.json`.
The calling skill validates the generated artifact with:

```bash
$FORGE_CMD schema:validate --file .forge/scenarios.json
```

Do not read `.forge/progress.json`, update task state, or edit config.
...
|------|----------|-------------|
| explicit `<spec_path>` from the calling skill | **Yes** | Superpowers brainstorming output, typically at `docs/superpowers/specs/YYYY-MM-DD-<feature>-design.md` |
| explicit `<feature_slug>` from the calling skill | **Yes** | URL-safe feature slug generated by `/start`; used in the `feature` field |
| Mockup HTML | No | Visual mockup if the feature involves UI (path may be referenced inside the spec) |

Read the explicit spec file in full before generating scenarios. If the caller
does not provide either required input, stop and ask for the missing value.
...
1. Read the file at explicit `<spec_path>` in full.
2. Extract:
...
| `source` | string | Explicit path to the design spec |
...
| Condition | Action |
|-----------|--------|
| Explicit `<spec_path>` missing | Return error: "spec_path is required. Run /start brainstorming first or provide the spec path." |
| Explicit `<feature_slug>` missing | Return error: "feature_slug is required. Provide the slug from /start." |
| Spec file at `<spec_path>` does not exist | Return error: "Spec file not found at `<spec_path>`." |
